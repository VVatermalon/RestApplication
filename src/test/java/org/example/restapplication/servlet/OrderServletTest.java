...
class OrderServletTest {
    ...
    private static final String STATUS_DEFAULT = "IN_PROCESS";
    ...
    @BeforeEach
    void setUp() {
        servlet = new OrderServlet(service, jsonMapper);
    }

    @Test
    void doGetOrderNotFound() throws IOException, ServiceException {
    ...
    void doGetFoundOrder() throws IOException, ServiceException {
        when(request.getParameter(ORDER_ID)).thenReturn(UUID_DEFAULT);
        when(service.findById(UUID.fromString(UUID_DEFAULT))).thenReturn(Optional.of(ORDER_DTO_DEFAULT));
    ...
    void doPutWithoutParameters() throws IOException {
        ...
        verify(request).getParameter(ORDER_ID);
        verify(request).getParameter(ORDER_STATUS);
        verify(response, times(1)).sendError(HttpServletResponse.SC_BAD_REQUEST);
    }
    ...
    void doPutWrongOrderId() throws IOException {
        ...
        when(request.getParameter(ORDER_STATUS)).thenReturn(STATUS_DEFAULT);
    ...
    @Test
    void doPutWrongStatus() throws IOException {
        when(request.getParameter(ORDER_ID)).thenReturn(UUID_DEFAULT);
        when(request.getParameter(ORDER_STATUS)).thenReturn(UUID_DEFAULT);
    ...
    void doPutOk() throws IOException, ServiceException {
        when(request.getParameter(ORDER_ID)).thenReturn(UUID_DEFAULT);
        when(request.getParameter(ORDER_STATUS)).thenReturn(STATUS_DEFAULT);
        when(service.update(any())).thenReturn(Optional.of(ORDER_DTO_DEFAULT));
    ...
    @Test
    void doDeleteNotExistingOrder() throws IOException, ServiceException {
        when(request.getParameter(ORDER_ID)).thenReturn(UUID_DEFAULT);
    ...
    void doDeleteOk() throws IOException, ServiceException {
        when(request.getParameter(ORDER_ID)).thenReturn(UUID_DEFAULT);
        when(service.delete(UUID.fromString(UUID_DEFAULT))).thenReturn(Optional.of(ORDER_DTO_DEFAULT));
